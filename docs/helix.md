...
`Fresh` returns an unexpired hit or fetches; `Refresh` bypasses lookup; `AllowStale` may return an expired hit with explicit `Freshness::Stale` and age. No silent stale-on-error or background revalidation occurs. Invalidation is explicit by class or whole cache. Future consumers can share metadata and choose refresh/stale behavior without building another authoritative frontend cache.
