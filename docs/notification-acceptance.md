...
## Real followed-live acceptance when behavior changes

The synthetic target cannot establish account/session behavior. For changes to monitoring, native delivery or lifecycle, use an installed build and a real followed-channel transition on affected platforms:

- Confirm startup, Resume and network/suspend recovery establish quiet baselines; a failed or partial scan is not an offline transition and must not replay an old notification.
- Confirm one genuinely new stream produces the expected channel notification, repeated polls or a brief disappearance do not duplicate it, and channel notification overrides behave as saved.
- Click while hidden or minimized: restore and select the original-session channel once, without autoplay. Pause, logout, re-login (including the same account under a new session) and Quit must retire old-session actions; delayed clicks cannot navigate another session.
- Where the OS retains notification history, distinguish natural banner timeout from explicit dismissal and expiry. Verify bounded retention/cleanup, later delivery after entries are cleared, and no takeover by a second app instance.
- Confirm explicit Quit stops owned playback and monitoring, removes retained actions and cannot be reversed by an old click. Record desktop notification-server/permission differences and any untested architecture.
