...
export function UpdateAwareness() {
  ...
  const run = async (action: () => Promise<void>) => {
    ...
    inFlight.current = true; revision.current++; setBusy(true); setError(null);
    try { await action(); } catch { if (mounted.current) setError("The update action could not be completed. Try again later."); }
  ...
  return <section className="setting-group update-awareness" aria-label="Update awareness">
    ...
    <div className="settings-save">
      <button type="button" disabled={busy || status.phase === "checking"} onClick={() => { void run(async () => { const value = await api.checkUpdates(); if (mounted.current) setStatus(value); }); }}>Check for updates</button>
      {status.phase !== "not_checked" && <button type="button" disabled={busy || status.phase === "checking"} onClick={() => { void run(async () => { const value = await api.refreshUpdates(); if (mounted.current) setStatus(value); }); }}>Refresh update check</button>}
      {status.latestVersion && <button type="button" disabled={busy} onClick={() => { void run(async () => { await api.openUpdateRelease(); }); }}>View release</button>}
    </div>
