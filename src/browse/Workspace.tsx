...
export const BrowserWorkspace = memo(function BrowserWorkspace({ sessionId, onAuthLost, watch, pending, actionsRef, settingsRevision }: { sessionId: string; onAuthLost: () => void; actionsRef: Ref<BrowserActions>; settingsRevision: number } & Watch) {
  ...
  return <div className="workspace">
    ...
    <main className="browse-content" ref={content}>
      ...
      {route.kind === "live" && <Streams key="live" mode="live" context={context} links={links} />}
      {route.kind === "categories" && <Categories context={context} links={links} />}
      {route.kind === "category" && <Category key={route.id} id={route.id} context={context} links={links} />}
...
function Streams({ mode, context, links }: { mode: "live" | "followed"; context: QueryContext; links: Links }) {
